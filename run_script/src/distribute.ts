...
const distribute = () => {
    ...
    const getArgs = async () => {
        ...
        const isArgsValid = (function() {
            ...
            if (['patch', 'minor', 'major'].indexOf(argv.level) == -1)
                return false;
            return true;
        })();
        ...
        if (!isArgsValid) {
            const question = [
                ...
                },
                {
                    ...
                    default: false,
                    message: util.chalk
                        .redBright`do you really want to publish?`
                }
            ];
    ...
    const updateVersion = async (NO: number, versionLv: string) => {
        console.log(
            util.chalk.redBright(`${NO} try updating version of ${versionLv}`)
        );
    ...
    const collectFiles = async (NO: number) => {
        console.log(
            util.chalk.redBright(`${NO} start collecting template files...`)
        );
        await util.copyDir(
            util.path.resolve(distTemplateDir),
            util.path.resolve(curDir, 'dist')
        );
        process.chdir(curDir);
        console.log(
            util.chalk.redBright(`${NO} start collecting dist files...`)
        );
        ...
        try {
            ...
            console.log(
                util.chalk.redBright(readmePath),
                'could not be copied:' + ex
            );
        }
    };
    ...
    const main = async () => {
        let { publish, level } = await getArgs();
        await build(1);
