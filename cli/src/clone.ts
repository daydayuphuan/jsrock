...
import { util } from '@jsrock/run_script';
...
const clone = () => {
    ...
    const fixPackageJson = async (
        ...
        let packagejson = JSON.parse(
            await util.readFile(packagejsonPath, 'utf8')
        );
    ...
    const fixProjectConfig = async (fpath: string, projectName: string) => {
        ...
        let fixStrProjectConfig = strProjectConfig.replace(
            /moduleName: '(.*)'/,
    ...
    const cloneProject = async (projectName: string, type: template_t) => {
        ...
        // clone for lib
        let source = util.path.dirname(
            require.resolve(`${tempalteRoot}/${type}_template/package.json`)
        );
        ...
        let packagejsonDistPath = util.path.resolve(
            destination,
            distributeTemplate,
            'package.json'
        );
        await fixPackageJson(packagejsonDistPath, projectName);
        let projectConfgPath = util.path.resolve(
            destination,
            'project.config.ts'
        );
        ...
        console.log(
            util.chalk.yellow(
                `cloning ${projectName} finished at \n${destination}`
            )
        );
    };

    const main = async () => {
        let { name, type } = await getArgs();
